...
namespace CodeVrikATS.UI.Controllers
{
    public class UtilityController : Controller
    {
        ...
        public UtilityController(WebApiHelper webApiHelper, CommonService common)
        {
        ...
        [HttpGet]
        public async Task<IActionResult> ReCalculatedPunchData()
        {
            var response = await _webApiHelper.GetAsync<string>(_apiHelper.ReCalculatePunchDataApi);

            if (response != null && response.Success)
            {
                return Json(new { success = true, message = "Punch Data ReCalculated Successfully"});
            }

            return Json(new { success = false, message = response?.Message ?? "Something went wrong." });
        }
