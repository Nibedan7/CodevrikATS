...
namespace CodeVrikATS.UI.Controllers
{
    public class EmployeeController : AuthController
    {
        ...
        public async Task<IActionResult> GetEmpAssByEmpCode(int empCode)
        {
            ...
            employeeAssessmentEntity.Reviewers = reviewer.Data;

            return PartialView("/Views/Employee/AddEditEmployeeAssessment.cshtml", employeeAssessmentEntity);
        }

        [HttpPost]
        public async Task<IActionResult> StartEmployeeAssessment(EmployeeAssessmentEntity request)
        {
            if (request == null)
            {
                return BadRequest("Invalid Data");
            ...
            if (ModelState.IsValid)
            {
                var response = await _webApiHelper.PostAsync<EmployeeAssessmentEntity>(_apiHelper.StartEmployeeAssessmentApi, request);
                if (response.Success)
                {
                    await _webApiHelper.PostAsync<dynamic>(_apiHelper.AppraisalMailApi, request);
        ...
        [HttpPost]
        public async Task<IActionResult> SaveEmployeeAssessment(EmployeeAssessmentEntity request)
        {
            if (request == null)
            {
                return BadRequest("Invalid Data");
            ...
            if (ModelState.IsValid)
            {
                var response = await _webApiHelper.PostAsync<EmployeeAssessmentEntity>(_apiHelper.SaveEmployeeAssessmentApi, request);
                if(request.Reviewer2Id != null && response.Success)
                {
                    await _webApiHelper.PostAsync<dynamic>(_apiHelper.AppraisalMailToReviewer2, request);
                }
                return Json(response);
            }
            return BadRequest("Model validation failed.");
