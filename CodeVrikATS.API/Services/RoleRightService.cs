...
namespace CodeVrikATS.API.Services
{
    public class RoleRightService
    {
        ...
        public async Task<ApiResponse<IEnumerable<RoleRightEntity>>> GetRoleRightList(int RoleId)
        {
            try
            {
                ...
                var roles = await _genericRepository.GetAllAsync<RoleRightEntity>("GetRoleRightList", parameters);
        ...
        public async Task<ApiResponse<string>> UpdateRoleRight(RoleRightEntity roleRightEntity)
        {
            try
            {
                var parameters = new SqlParameter[]
