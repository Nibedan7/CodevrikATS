...
namespace CodeVrikATS.API.Controllers
{
    ...
    public class ProjectTrackerApiController : ControllerBase
    {
        ...
        [HttpPost("UpdatePtColor")]
        public async Task<IActionResult> UpdatePtColor([FromBody] ProjectTrackerCellAddEdit request)
        {

            var response = await _projectTrackerService.UpdatePtColor(request);
            if (!response.Success)
            {
                return BadRequest(response);
            }
